...
const { ODARKA_TELEGRAM_TOKEN } = process.env;
if (!ODARKA_TELEGRAM_TOKEN) {
	console.error('No ODARKA_TELEGRAM_TOKEN provided.');
	process.exit(1);
}
...
(async () => {
	try {
		...
		await startTelegramBot(controller, ODARKA_TELEGRAM_TOKEN, parseInt(PETLYURYK_PRIVACY_EXPIRE));
